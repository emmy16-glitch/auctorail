...
for(let attempt=1; attempt<=3 && !tx; attempt++){
  ...
  try{
    ...
    if(tx){ await p.screenshot({path:OUT+'/success.png'}); log('TX=',tx);
      try{ const vp=p.locator('button',{hasText:'VIEW PROOF'}).first(); await vp.click(); await p.waitForTimeout(900);
        const d=await p.evaluate(()=>document.body.innerText); fs.writeFileSync(OUT+'/proof.txt', d);
        await p.screenshot({path:OUT+'/success-proof.png'}); log('PROOF captured'); }catch(e){ log('proof err', e.message.split('\n')[0]); }
    }
