...
const receiptFile = jsonFiles(path.join("data", "receipts"))[0];
...
if (!bundle) {
  ...
  if (rejected.length > 0) {
    console.log("Rejected paid evidence (correctly quarantined):");
    for (const entry of rejected) {
      const value = entry.value;
      const result = value?.rawResponse?.result ?? {};
      const returnedChain =
        result?.meta?.chainId ?? result?.chainId ?? result?.chain ?? result?.network;
      console.log(
        `  ${value?.miner?.name ?? "unknown"} (${value?.miner?.id ?? "?"}) ` +
        `→ ${value?.rejection?.code ?? "rejected"}`
      );
      if (returnedChain !== undefined) {
        console.log(`    returned chain/network: ${String(returnedChain)}`);
      }
  ...
  printLine("Observed total raw", (acceptedSpend + rejectedSpend).toString());
  console.log("");

  if (receipt) {
    printLine("Latest receipt", short(receiptFile));
