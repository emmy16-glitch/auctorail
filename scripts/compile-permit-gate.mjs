...
const CONTRACT_NAME = "AuctorailPermitGate";
const SOURCE_NAME = "AuctorailPermitGate.sol";
...
const artifactDir = path.join(root, "artifacts", "permit-gate");
const artifactPath = path.join(artifactDir, `${CONTRACT_NAME}.json`);
const manifestPath = path.join(artifactDir, `${CONTRACT_NAME}.build.json`);
...
function assertNativeCompilerPlatform() {
  ...
  throw new Error(
    ...
    "canonical Auctorail permit-gate compilation runs on linux-amd64"
  );
}
...
async function ensureCompiler() {
  ...
  const temporaryPath = path.join(
    toolsDir,
    `${COMPILER_BINARY_NAME}.${process.pid}.tmp`
  );
...
if (!contract?.evm?.bytecode?.object || !contract?.evm?.deployedBytecode?.object) {
  throw new Error("compiled_permit_gate_artifact_missing");
}
...
const manifest = {
  schemaVersion: "auctorail.permit-gate-build.v1",
  contractName: CONTRACT_NAME,
  source: {
  ...
  artifact: {
    path: `artifacts/permit-gate/${CONTRACT_NAME}.json`,
